...
| ship-gate | Require green local checks plus live Outplane verification before every ship. | `scripts/check-ship.mjs`; `scripts/check-git-ship-state.mjs`; `scripts/check-live-deploy.mjs`; `server/package.json`; `docs/compliance-audit.md`; `README.md` | Proven | Keep every new compliance audit invariant wired into `npm --prefix server run ship:check`. |
