...
| 38 | UX |**Staggered card list animation** | Cards fade in with sequential delays (0.03s × index) using CSS nth-child rules, up to 20 items. |
| 39 | SEARCH |**Command palette fuzzy search** | Cmd+K matches non-consecutive chars — "st" finds "Start", "cl" finds "Calendar". |
| 40 | GAME |**Achievement badges** | 6 unlockable badges in sidebar footer — first done, 10 tasks, 7-day streak, etc. |
| 41 | UX |**Deadline countdown (AR)** | Cards show "2 days overdue" (red), "Due today" (amber), urgency from `pack.due` vs current date. |
| 42 | UX |**Inline editing on cards (AM)** | Double-click title or owner to edit inline. Enter saves, Escape cancels, blur auto-saves. |
| 43 | GAME |**Achievement badges (AW)** | 6 unlockable emoji badges in sidebar footer — first done, 10 tasks, 7-day streak, all fields, 20 items, blocker cleared. Persistent in localStorage. |
| 44 | UX |**Gantt click-to-navigate (G)** | Gantt bars and labels navigate to the pack on click. Delegated handler, CSSStyleSheet-safe. |
| 45 | UX |**Offline sync indicator (BB)** | Green/red dot in sidebar achievements area — updates on online/offline events. |
| 46 | NLP |**Natural-language relative dates (Q)** | Parser handles "this Friday", "end of month", "end of this month". |
| 47 | UX |**Audio cue on toasts (S)** | Subtle beep via Web Audio API — low tone on error, short tick on info/success. |
| 48 | DATA |**Print-friendly work list (T)** | Enhanced @media print: clean card borders, page-break avoidance, full-width layout. |
| 49 | UX |**Snooze / defer (AA)** | Snooze 1d/3d/7d buttons in card support, pushes due date forward with activity log entry. |
| 50 | UI |**Custom accent color (AP)** | Color picker in Settings overrides --cockpit-accent, persisted in localStorage. |
| 51 | UX |**Desktop deadline notifications (BV)** | System notification for items due today on startup. Requests permission. |
| 52 | UX |**Energy level (BN)** | Low/Medium/High per work item, filter chips, icon on cards. |
...
| DP | UI |**Dark mode contrast audit** | LARGELY DONE 2026-07-12 at the token level: `scripts/check-contrast-apca.mjs` gates every real token pairing across all 5 themes + 3 seasonal accents using APCA (Lc floors 75/60/30), which replaced the old ≥ 4.5:1 target and caught failing muted text and accent labels in every dark theme. Remaining scope: component-level pairs not expressed as `--cockpit-*` tokens (hardcoded one-offs), if any survive. |
