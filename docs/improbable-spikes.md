...
| DP | UI |**Dark mode contrast audit** | Systematic scan of every component across all 5 themes (light, dark, forest, ocean, sepia) for color contrast ratio ≥ 4.5:1. Fix method cards, state pills, field labels, help text, and any other components that fail WCAG AA. Currently known: method-card title contrast in dark mode (fix applied). Likely more. |
