...
| Oversized JSON bodies can consume backend memory | Fixed | Body routes reject declared oversized JSON before reading the request stream, and the local boundary plus deploy-config gates prove that ordering; streamed oversize reads return `413` once JSON exceeds 1 MiB, and local/live gates prove oversized state writes are rejected before storage |
