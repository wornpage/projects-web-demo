...
scalar JSON, and array JSON state writes are rejected, confirms oversized keyed
state snapshots, duplicate work ids, invalid work items, oversized
...
| Malformed JSON snapshots can wipe a keyed state row | Fixed | `PUT /api/state` requires a JSON object snapshot; scalar and array payloads return `400` and leave the keyed row unchanged |
