...
| Oversized JSON bodies can consume backend memory | Fixed | Body routes reject declared oversized JSON before reading the request stream, stop streamed reads and return `413` once a JSON body exceeds 1 MiB, and local/live gates prove oversized state writes are rejected before storage |
