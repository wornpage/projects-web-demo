...
try {
  ...
  const unkeyedPacksStatus = await readStatus("/api/packs");
  const unkeyedCommandPreviewStatus = await readStatus("/api/packs/source-folder-audit/command");
  ...
  check("hosted pack list rejects missing client key", unkeyedPacksStatus === 400, unkeyedPacksStatus);
  ...
  check("hosted browser-row write preserves backend-owned status", /backend row/u.test(browserStatusOverwriteWrite.status || "") && /backend row/u.test(resetStateAfterBrowserWrite.status || ""), `${browserStatusOverwriteWrite.status || "missing"} / ${resetStateAfterBrowserWrite.status || "missing"}`);
